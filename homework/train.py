...
def train(iterations, batch_size=64, log_dir=None):
    ...
    for iteration in range(iterations):
        ...
        if iteration % 5 == 0:
            ...
            model_outputs = model(batch_inputs.cuda())
        ...
        if iteration%100 ==0:
            v_acc_val = accuracy(model_outputs, batch_labels)
            file = 'convnet'+str(iteration)+'.th'
            print("SAVING MODEL "+ str(v_acc_val)+'convnet'+str(iteration)+'.th')
            torch.save(model.state_dict(), os.path.join(dirname, file)) # Do NOT modify this line
